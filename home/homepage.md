---
banner: "![[1.jpg]]"
cssclasses:
  - dashboard
banner_y: 0.191
---

# :oleg_python: <span style="color: green">**Python**</span>
- #### Основы Python
	- [Установка и настройка Python](../Установка%20и%20настройка%20Python.md)
	- [Основы Python (PEP-8 и PEP-257)](../Основы%20Python%20(PEP-8%20и%20PEP-257).md)
	- [Ветвление](../Ветвление.md)
	- [Циклы](../Циклы.md) 
- #### Простые типы данных
	- [Простые типы данных](../Простые%20типы%20данных.md)
	- [Аннотация типов](../Аннотация%20типов.md)
	- [Атрибуты и методы объектов](../Атрибуты%20и%20методы%20объектов.md)
	- [Простые объекты](../Простые%20объекты.md)
	- [Математика в Python](../Математика%20в%20Python.md)
- #### Коллекции
	- [Списки, [list]](../Списки,%20[list].md)
	- [Строки, str](../Строки,%20str.md)
	- [Кортеж, (tuple)](../Кортеж,%20(tuple).md)
	- [Словарь, {dict}](../Словарь,%20{dict}.md)
	- [Множества {set})](../Множества%20{set}%20и%20({frozenset}).md)
	- [Классы bytes и bytearray](../Классы%20bytes%20и%20bytearray.md)
- #### Итераторы и генераторы
	- [Однострочники](../Однострочники.md)
	- [Итераторы](../Итераторы.md)
	- [Генераторы](../Генераторы.md)
	- [Создание функции генератора](../Создание%20функции%20генератора.md)
- #### Функции
	- [Создание функций](../Создание%20функций.md)
	- [Встроенные функции](../Встроенные%20функции.md)
- #### Файлы и файловая система
	- [Работа с файлами](../Работа%20с%20файлами.md)
	- [Файловая система](../Файловая%20система.md)
- #### Сериализация
	- [JSON](../JSON.md)
	- [CSV](../CSV.md)
- #### Регулярные выражения
	- [Регулярные выражения](../Регулярные%20выражения.md)
	- [Примеры регулярок](../Примеры%20регулярок.md)
- #### Фреймворки Flask и FastAPI
	- [Знакомство с Flask](../Знакомство%20с%20Flask.md)
	- [Погружение во Flask](../Погружение%20во%20Flask.md)
- #### Модули
	- [Модули. import](../Модули.%20import.md)
	- [Свои и встроенные модули](../Свои%20и%20встроенные%20модули.md)
- #### Модули A-K
	- [APScheduler](../APScheduler.md)
	- [ARGPARSE](../ARGPARSE.md)
- #### Модули L-R
	- [LOGGING](../LOGGING.md)
	- [OS](../OS.md)
	- [PATHLIB](../PATHLIB.md)
	- [QUEUE](../QUEUE.md)
	- [RANDOM](../Модуль%20random.md)
	- [RE](../RE.md)
	- [REQUESTS](../REQUESTS.md)
- #### Модули S-T
	- [SCHEDULE](../SCHEDULE.md)
	- [SHUTIL](../SHUTIL.md)
	- [SOCKET](../SOCKET.md)
	- [SUBPROCESS](../SUBPROCESS.md)
	- [SYS](../Модуль%20sys.md)
	- [THREADING](../THREADING.md)
	- [TIME](../TIME.md)


# :oleg_linux: <span style="color: green">**Linux**</span> 
- #### Сеть
	- [netcat](../netcat.md)
	- 
	- Msfvenom

# :oleg_kalilinux: <span style="color: green">**Kali Linux Tools**</span> 
- #### Сеть
	- [Nmap](../Nmap.md)
	- [Masscan](../Masscan.md)
	- [CURL](../CURL.md)


# :luc_network:    <span style="color: green">**Network**</span> 

- #### Организация компьютерных сетей
	- Терминология сетей
	- Модель ISO OSI
	- Модель и стек TCP/IP 
	- Анализатор сети Wireshark
	- Терминология сетей
	- Модель ISO OSI
	- Модель и стек TCP/IP 
	- Анализатор сети Wireshark
- #### Прикладной уровень
	- Протокол HTTP
	- Система доменных имён DNS
- #### Транспортный уровень
	- Протокол UDP
	- Протокол TCP
	- Интерфейс сокетов
- #### Прикладной уровень
	- Протокол SMTP
	- Протокол POP3
	- Протокол IMAP
	- Протокол FTP



# :oleg_gnubash: <span style="color: green">**Bash**</span> 


# :oleg_hackaday: <span style="color: green">**Pentest**</span> 



# :luc_library:  <span style="color: green" class="plain-text"> [**Библиотека**](books.md) </span> 


# :obs_links_going_out: <span style="color: green">**Полезные ссылки**</span> 

- #### Курсы
	- [Профессия "Белый хакер" (cyber-ed)](https://learn.cyber-ed.ru/course/view.php?id=439)
	- [Комплексная настройка безопасности и анонимности (cyberyozh)](https://book.cyberyozh.com/ru/vstuplenie/kurs-kompleksnaya-nastrojka-bezopasnosti-i-anonimnosti)
	- [Специалист по кибербезопасности (cyberyozh)](https://edu.cyberyozh.com/courses/course-specialist-po-kiberbezopasnosti-s-nulya-dlya-jun/lesson)
	- [Proxy для анонимности (cyberyozh)](https://edu.cyberyozh.com/courses/course-proxy-dlya-anonimnosti/lesson)
	- [Администрирование Линукс](https://www.youtube.com/watch?v=5UJtJMTKZNg&list=PLYl91BhaOf-kkXWweBzgOw555he6S7SUs)
	- [Компьютерные сети (Созыкин)](https://www.youtube.com/playlist?list=PLtPJ9lKvJ4ojPWFLuUz6g8c73Ta45bUN8)
	- [Курс по Obsidian](https://www.youtube.com/watch?v=aeebp25l9Gg&list=PLrRc3UisLr6KVOYhzpSnywtHkCi2PEza5&index=2)

- #### Статьи
	- [Автоматизация конспектов для ленивых: Obsidian + Zotero / Хабр](https://habr.com/ru/companies/banki/articles/939120/)
	- [Управление знаниями в Obsidian](https://habr.com/ru/articles/710508/)
	- [VeraCrypt-инструкция](https://veracrypt.jp/ru/Beginner%27s%20Tutorial.html)